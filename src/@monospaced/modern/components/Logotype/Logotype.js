...
/**
 * Use `Logotype` to display the Monospaced brand logotype.
 */
const Logotype = ({ color = "currentColor", height = null, width = null }) => {
  if (height) {
    const heightUnit = height.replace(/[0-9.-]/g, "");
    width = `${parseFloat(height) * 7.154761905}${heightUnit}`;
  }
  return (
    <span className="Logotype" style={{ width }}>
      <span className="Logotype-inner">
        <svg
...
Logotype.colors = Object.keys(colors).filter(
  colorKey => !colorKey.includes("-rgb"),
);

Logotype.propTypes = {
  /**
   * Fill color of the Logotype SVG.
   */
  color: PropTypes.oneOfType([
    PropTypes.oneOf([`currentColor`]),
    PropTypes.oneOf(Logotype.colors),
  ]),

  /**
   * A valid CSS height value for the Logomark.
   */
  height: PropTypes.string,

  /**
   * A valid CSS width value for the Logomark. Ignored when height prop provided
   * (component calculates correct aspect ratio width for height).
   */
